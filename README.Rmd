...
* **`ted`**, decentralized credit between federal bodies (*termo de execução
  ...
  which this package no longer uses.
* **The older PostgREST endpoints** for special and fund-to-fund transfers on
  that same host. They are still up and still answer, but they are a different
  and largely superseded contract — different column names, a handful of
  columns each way, and a `historico_pagamento_especial` table that the new
