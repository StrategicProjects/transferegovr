...
sizes <- do.call(rbind, lapply(seq_len(nrow(tg_tables())), function(i) {
  row <- tg_tables()[i, ]
  data.frame(
    module = row$module,
    table = row$table,
    rows = tg_count(row$module, row$table)
  )
}))

sizes[order(-sizes$rows), ][1:5, ]
#>        module                          table    rows
#> 1 fundoafundo  gestao_financeira_lancamentos 1115444
#> 2 fundoafundo gestao_financeira_subtransacoes 377666
#> 3 transferen…    historico_pagamento_especial  281163
#> 4 fundoafundo           plano_acao_historico  183379
#> 5 transferen…                   meta_especial  156016
```
